...
public class RestaurantController {
  ...
  public ResponseEntity<?> getRestaurant(@PathVariable("id") String id) {
    ...
    if (existingRestaurant.isPresent()) {
      ...
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body("Restaurant with Id:" + id + " Does not exist");
    }
  }
  ...
  public ResponseEntity<?> addRestaurant(@RequestBody RestaurantDTO restaurantDTO) {
    Optional<Restaurant> existingRestaurant =
        service.getRestaurantByName(restaurantDTO.getRestaurantName());
    if (!existingRestaurant.isPresent()) {
      ...
      return ResponseEntity.status(HttpStatus.FOUND)
          .body("Restaurant " + restaurantDTO.getRestaurantName() + " already exist");
    }
  }
  ...
  public ResponseEntity<?> updateRestaurant(@RequestBody RestaurantDTO restaurantDTO) {
    ...
    if (existingRestaurant.isPresent()) {
      Restaurant restaurantMapped =
          this.mapper.mapperUpdate(existingRestaurant.get(), restaurantDTO);
      return ResponseEntity.status(HttpStatus.CREATED)
          .body(this.service.saveRestaurant(restaurantMapped));
    } else {
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body("Restaurant " + restaurantDTO.getId() + " does not exist");
    }
  }
  ...
  public ResponseEntity<?> deleteRestaurant(@PathVariable("id") String id) {
    ...
    if (existingRestaurant.isPresent()) {
      ...
      return ResponseEntity.status(HttpStatus.OK)
          .body("Restaurant with Id: " + id + " was successfully deleted");
    } else {
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body("Restaurant with Id:" + id + " Does not exist");
    }
