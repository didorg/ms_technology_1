...
public class OrderController {
  ...
  public OrderController(IOrderService service, OrderMapper mapper,
      OrderResponseMapper mapperResponse, IApiService apiService) {
  ...
  public ResponseEntity<?> getOrder(@PathVariable("orderNumber") String orderNumber)
    ...
    if (existingOrder.isPresent()) {
      ...
      OrderResponse orderResponse =
          this.mapperResponse.mapperOrderResponse(order, apiCustomer, apiRestaurant);
      return ResponseEntity.status(HttpStatus.OK).body(orderResponse);
    } else {
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body("Order with Id:" + orderNumber + " Does not exist");
    }
  }
  ...
  public ResponseEntity<?> updateOrder(@RequestBody OrderDTO orderDTO) {
    ...
    if (existingOrder.isPresent()) {
      ...
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body("Order " + orderDTO.getId() + " does not exist");
    }
  }
  ...
  public ResponseEntity<?> deleteOrder(@PathVariable("id") String id) {
    ...
    if (existingOrder.isPresent()) {
      ...
      return ResponseEntity.status(HttpStatus.OK)
          .body("Order with Id: " + id + " was successfully deleted");
    } else {
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
